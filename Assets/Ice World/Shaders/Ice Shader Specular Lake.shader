// Upgrade NOTE: upgraded instancing buffer 'NatureManufactureShadersIceShaderSpecularLake' to new syntax.

Shader "NatureManufacture Shaders/Ice Shader Specular Lake"
{
	Properties
	{
	...
	SubShader
	{
		...
		UNITY_INSTANCING_BUFFER_START(NatureManufactureShadersIceShaderSpecularLake)
			UNITY_DEFINE_INSTANCED_PROP(fixed4, _RefractionColor)
#define _RefractionColor_arr NatureManufactureShadersIceShaderSpecularLake
			UNITY_DEFINE_INSTANCED_PROP(fixed, _SpecularPower)
#define _SpecularPower_arr NatureManufactureShadersIceShaderSpecularLake
			UNITY_DEFINE_INSTANCED_PROP(fixed, _Smoothness)
#define _Smoothness_arr NatureManufactureShadersIceShaderSpecularLake
			UNITY_DEFINE_INSTANCED_PROP(fixed, _IceAmountOpacity)
#define _IceAmountOpacity_arr NatureManufactureShadersIceShaderSpecularLake
		UNITY_INSTANCING_BUFFER_END(NatureManufactureShadersIceShaderSpecularLake)
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			fixed4 screenColor305 = tex2D( GrabScreen1, ( (ase_screenPosNorm).xy + (( tex2DNode17 * _RefractionPower )).xy ) );
			float decodeFloatRGBA303 = DecodeFloatRGBA( ( 1.0 - screenColor305 ) );
			fixed4 temp_cast_4 = (pow( ( decodeFloatRGBA303 - 0.1 ) , 4.0 )).xxxx;
			float decodeFloatRGBA295 = DecodeFloatRGBA( temp_cast_4 );
			float lerpResult298 = lerp( 0.0 , _ShadowPower , decodeFloatRGBA295);
			float clampResult299 = clamp( lerpResult298 , 0.0 , 1.0 );
			float4 lerpResult294 = lerp( lerpResult279 , _RefractionColor_Instance , clampResult299);
			o.Emission = lerpResult294.rgb;
			fixed _SpecularPower_Instance = UNITY_ACCESS_INSTANCED_PROP(_SpecularPower_arr, _SpecularPower);
			fixed3 temp_cast_6 = (_SpecularPower_Instance).xxx;
